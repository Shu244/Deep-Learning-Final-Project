...
class TwoStageDetector(BaseDetector, RPNTestMixin,
    ...
    def __init__(self,
        ...
        if track_head is not None:
            ...
            self.in_last_frame=None
        if mask_head is not None:
            self.mask_roi_extractor = builder.build_roi_extractor(
                mask_roi_extractor)
            self.mask_head = builder.build_head(mask_head)
        
        self.total_bboxes = None
        self.total_roi_feats = None
        self.total_det_labels = None
        self.total_num=0
    ...
    def forward_train(self,
        ...
        if self.with_bbox:
            ...
            delta_frame = img_meta[0]['delta_frame']
            match_score, shift,IoUs2,distances_xy= self.track_head(bboxes, ref_bboxes,
                                          bbox_feats, ref_bbox_feats, 
                                          bbox_img_n, ref_bbox_img_n,
                                         delta_frame=delta_frame)
            loss_match = self.track_head.loss(match_score, shift,IoUs2,distances_xy, ids, id_weights)
    ...
    def simple_test_bboxes(self,
        ...
        det_bboxes, det_labels = self.bbox_head.get_det_bboxes(
            ...
            rescale=rescale,
            cfg=rcnn_test_cfg)
        # only choose bboxes with confidence >= 0.6
        ind=[i for i,bboxes in enumerate(det_bboxes) if bboxes[4]>=0.6]
        det_bboxes=det_bboxes[ind]
        det_labels=det_labels[ind]
        ...
        if is_first or (not is_first and self.prev_bboxes is None):
            det_obj_ids = torch.arange(det_bboxes.size(0),device=torch.cuda.current_device())+1
            # save bbox and features for later matching
            self.total_bboxes = det_bboxes
            self.total_roi_feats = det_roi_feats
            self.total_det_labels = det_labels
            
            self.prev_bboxes = self.total_bboxes[det_obj_ids-1]
            self.prev_roi_feats = self.total_roi_feats[det_obj_ids-1]
            self.prev_det_labels = self.total_det_labels[det_obj_ids-1]
            self.in_last_frame = det_obj_ids
            print(det_obj_ids)
            ...
            match_score = self.track_head(det_bboxes[:,:4], self.prev_bboxes[:,:4],
                                      det_roi_feats, self.prev_roi_feats,
                                      bbox_img_n, prev_bbox_img_n)[0]
            
            match_score=torch.stack(match_score,dim=0)
            ...
            comp_scores = self.track_head.compute_comp_scores(match_logprob, 
                ...
                add_bbox_dummy=True)
            '''
            comp_scores=comp_scores.cpu().numpy().astype(np.int32)
            comp_scores = comp_scores * self.in_last_frame
            self.in_last_frame = np.zeros(1+self.prev_bboxes.size(0))
            self.in_last_frame[0]=1
            match_likelihood = comp_scores.max(axis =1)
            match_ids = comp_scores.argmax(axis =1)
            print(match_likelihood,match_ids)
            '''
            match_ids = comp_scores
            # translate match_ids to det_obj_ids, assign new id to new objects
            # update tracking features/bboxes of exisiting object, 
            # add tracking features/bboxes of new object
            
            #match_ids = match_ids.cpu().numpy().astype(np.int32)
            det_obj_ids = torch.ones((match_ids.shape[0]), device=torch.cuda.current_device()) * (-1)
            #best_match_scores = np.ones((self.prev_bboxes.size(0))) * (-100)
            for idx, match_id in enumerate(match_ids):
                if match_id == 0:
                    # add new object
                    det_obj_ids[idx] = self.total_roi_feats.size(0)+1
                    self.total_roi_feats = torch.cat((self.total_roi_feats, det_roi_feats[idx][None]), dim=0)
                    self.total_bboxes = torch.cat((self.total_bboxes, det_bboxes[idx][None]), dim=0)
                    self.total_det_labels = torch.cat((self.total_det_labels, det_labels[idx][None]), dim=0)
                    #self.in_last_frame = np.append(self.in_last_frame,1)
                else:
                    ...
                    det_obj_ids[idx] = self.in_last_frame[match_id-1]
                        #best_match_scores[obj_id] = match_score
                    # udpate feature
                    self.total_roi_feats[match_id-1] = det_roi_feats[idx]
                    self.total_bboxes[match_id-1] = det_bboxes[idx]
                    #self.in_last_frame[match_id] = 1
            det_obj_ids=det_obj_ids.type(torch.int64)
            self.prev_bboxes = self.total_bboxes[det_obj_ids-1]
            self.prev_roi_feats = self.total_roi_feats[det_obj_ids-1]
            self.prev_det_labels = self.total_det_labels[det_obj_ids-1]
            self.in_last_frame = det_obj_ids
            print(self.in_last_frame)
            print(len(self.prev_roi_feats),len(self.total_bboxes))
            print('~~~~~~~~~~~~~~~~~~~~~~~~~')
        return det_bboxes, det_labels, det_obj_ids
